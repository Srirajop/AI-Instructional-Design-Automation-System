...
## AI Instructional Designer — v3.1
### Engineered by Sriraj Pillai & Priyanshu Sharma @ EwandzDigital
...
- **Zero-Truncation Storyboards**: Storyboards are generated in **sequential API loops** — one call per module — ensuring the model never "runs out of budget" or ignores later sections.
- **High-Precision Design Docs**: Uses "One-Shot Gold Standard" prompting to generate complex, structured tables with 100% formatting accuracy.
- **Multi-Source Extraction**: Native drivers for PDF, DOCX, XLSX, PPTX, YouTube Transcripts, and Website Scraping.
- **Persistent Storage**: All generated content is stored as `LONGTEXT` in MySQL/SQLite for reliable retrieval.
...
| POST | `/api/extract/url` | Scrape text content from a web URL |
| POST | `/api/extract/youtube` | Fetch transcript from a YouTube video URL |
...
### Design Document (High-Precision Architecture)

The design document generator uses a **One-Shot Gold Standard** prompting technique. By providing a curated example of a cybersecurity module, the AI learns the exact density, tone, and formatting required for any complex topic.

```
generate_design_document(api_key, intake_data, content)
    │
    ├─► Structural Framing
    │   └─ Metadata (Project Info, Overview, Objectives)
    │
    ├─► Gold Standard Injection
    │   └─ "Cybersecurity Table Example" mapped to current topic
    │
    └─► High-Token Output
        └─ Llama-3.3-70b-Versatile generates the full table in one pass
           (Target: ~2,500 - 4,000 tokens)
```

**Estimated generation time:** ~15–25 seconds.
